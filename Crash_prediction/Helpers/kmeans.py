...
def _tolerance(X, tol):
    ...
    if sp.issparse(X):
        variances = mean_variance_axis(X)[1]
    else:
...
def k_means(X, n_clusters, init='k-means++', precompute_distances=True,
            ...
            tol=1e-4, random_state=None, copy_x=True, n_jobs=1, distance_function=euclidean_distances):
    ...
    if n_jobs == 1:
        ...
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            delayed(_kmeans_single)(X, n_clusters, max_iter=max_iter,
                                    ...
                                    random_state=seed, distance_function=distance_function)
            for seed in seeds)
...
class KMeans(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def fit(self, X, y=None, distance_function=euclidean_distances):
        ...
        self.cluster_centers_, self.labels_, self.inertia_ = k_means(
            ...
            n_jobs=self.n_jobs, distance_function=distance_function)
        return self
    ...
    def _transform(self, X, distance_function=euclidean_distances):
        """guts of transform method; no input validation"""
        return distance_function(X, self.cluster_centers_)

    def predict(self, X, distance_function=euclidean_distances):
        ...
        X = self._check_test_data(X)
        x_squared_norms = row_norms(X, squared=True)
        return _labels_inertia(X, x_squared_norms, self.cluster_centers_, distance_function=distance_function)[0]

    def score(self, X, distance_function=euclidean_distances):
        ...
        X = self._check_test_data(X)
        x_squared_norms = row_norms(X, squared=True)
        return -_labels_inertia(X, x_squared_norms, self.cluster_centers_, distance_function=distance_function)[1]
...
def _mini_batch_step(X, x_squared_norms, centers, counts,
                     ...
                     verbose=False, distance_function=euclidean_distances):
    ...
    nearest_center, inertia = _labels_inertia(X, x_squared_norms, centers,
                                              distances=distances, distance_function=distance_function)
...
class MiniBatchKMeans(KMeans):
    ...
    def fit(self, X, y=None, distance_function=euclidean_distances):
        ...
        for init_idx in range(n_init):
            ...
            # TODONOT: once the `k_means` function works with sparse input we
            ...
            _, inertia = _labels_inertia(X_valid, x_squared_norms_valid,
                                         cluster_centers, distance_function=distance_function)
            if self.verbose:
        ...
        if self.compute_labels:
            self.labels_, self.inertia_ = self._labels_inertia_minibatch(X, distance_function=distance_function)

        return self

    def _labels_inertia_minibatch(self, X, distance_function=euclidean_distances):
        ...
        results = [_labels_inertia(X[s], x_squared_norms[s],
                                   self.cluster_centers_, distance_function=distance_function) for s in slices]
        labels, inertia = zip(*results)
        return np.hstack(labels), np.sum(inertia)

    def partial_fit(self, X, y=None, distance_function=euclidean_distances):
        ...
        X = check_array(X, sparse_format="csr", copy=False)[0]
        n_samples, n_features = X.shape
        if hasattr(self.init, '__array__'):
            self.init = np.ascontiguousarray(self.init, dtype=np.float64)
        ...
        if (not hasattr(self, 'counts_')
            ...
            self.cluster_centers_ = _init_centroids(
                ...
                x_squared_norms=x_squared_norms, init_size=self.init_size, distance_function=distance_function)
        ...
        if self.compute_labels:
            self.labels_, self.inertia_ = _labels_inertia(
                X, x_squared_norms, self.cluster_centers_, distance_function=distance_function)
    ...
    def predict(self, X, distance_function=euclidean_distances):
        ...
        self._check_fitted()
        X = self._check_test_data(X)
        return self._labels_inertia_minibatch(X, distance_function=distance_function)[0]