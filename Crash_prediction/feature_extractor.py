...
from Crash_prediction.individual_mobility_network import entropy as calculate_entropy
...
def get_trjectory_features(trajectories):
    ...
    for traj in trajectories.values():
        ...
        day = pd.Timestamp(traj.start_time(), unit='s').date()
        # time = traj.start_time().time()
        time = pd.Timestamp(traj.start_time(), unit='s').time()
...
def get_events_features(events):
    ...
    for event in events.values():
        # day = event['date'].date()
        day = pd.Timestamp(event['date'], unit='s').date()
        # time = event['date'].time()
        time = pd.Timestamp(event['date'], unit='s').time()
...
def get_imn_temporal_features(imn_list, loc_dist_thr=100):
    ...
    # index_min = 0
    # imn_key0 = imn_keys_sorted[index_min]
    # while index_min < len(imn_keys_sorted) - 1 and imn_list[imn_key0]:
    #     index_min += 1
    #     imn_key0 = imn_keys_sorted[index_min]
    #
    # index_max = len(imn_keys_sorted) - 1
    # imn_key1 = imn_keys_sorted[index_max]
    # while index_max > 0 and imn_list[imn_key1]:
    #     index_max -= 1
    #     imn_key1 = imn_keys_sorted[index_max]
    #
    ...
    #
    # if index_min >= index_max or imn0 is None or imn1 is None:
...
# Added to support networkx 2.3 in 2.4
...
def get_imn_features(imn_list, event_traj2evntlist):
    ...
    for m0m1, imn in imn_list.items():
        ...
        if nx.is_connected(nx.to_undirected(graph)):
            ...
            Gc = max(connected_component_subgraphs(nx.to_undirected(graph)), key=len)
...
def extract_features_data(uid, data, quadtree, quadtree_features):
    ...
    for index, values in data.items():
        ...
        # features_names = {
        #     'traj': list(traj_features.keys()),
        #     'evnt': list(evnt_features.keys()),
        #     'imn': list(imn_features.keys()),
        #     'col': list(collective_features.keys()),
        # }
        # print(json.dumps(features_names))
